 ...
 * Set keys here only for local use, or pass them via constructor.
 */
const AI_CONFIG = {
    openai: {
        apiKey: null, // Set your OpenAI API key here or pass to constructor
        baseURL: "https://api.openai.com/v1",
        defaultModel: "gpt-4o-mini",
        defaultMaxTokens: 1000
    },
    claude: {
        apiKey: null, // Set your Anthropic API key here or pass to constructor
        baseURL: "https://api.anthropic.com/v1",
        defaultModel: "claude-3-5-sonnet-20241022",
        defaultMaxTokens: 1000,
        version: "2023-06-01" // Update this when using newer API versions
    }
};
...
class AIService {
    ...
    async analyzeImage(image, prompt, options = {}) {
        if (this.provider !== "openai") {
            throw new Error("Image analysis is currently only supported with OpenAI. Claude vision support coming soon.");
        }
...
function extractJSONFromResponse(response) {
    try {
        ...
        // Try to find JSON object in response using non-greedy match
        const jsonMatch = response.match(/\{[\s\S]*?\}/);
        if (jsonMatch) {
            try {
                return JSON.parse(jsonMatch[0]);
            } catch (e2) {
                // If still fails, try the first complete JSON object
